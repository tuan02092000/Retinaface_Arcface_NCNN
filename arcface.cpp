...
{
    std::string param_file = model_folder + "/mobilefacenet.param";
    std::string bin_file = model_folder + "/mobilefacenet.bin";
...
ncnn::Mat preprocess(ncnn::Mat img, FaceObject info)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        src[i] = info.landmark[i].x;
        src[i + 5] = info.landmark[i].y;
    }
